...
const TYPE_ICON_MAP: Record<string, React.ReactNode> = {
    ...
    default: <Syringe className="h-4 w-4" />,
};
...
export default function TreatmentsIndex({
    ...
    const getIcon = (type: string | null) =>
        TYPE_ICON_MAP[type || 'default'] || TYPE_ICON_MAP.default;
    ...
    const handleApprove = () => {
        ...
        router.post(
            treatmentsApprove.url(selectedTreatment.id),
            {},
            {
                onSuccess: () => {
                    ...
                    addToast({
                        message: 'Traitement approuvé',
                        type: 'success',
                    });
                    // Recharger les données du calendrier
                    router.reload({
                        only: ['treatments', 'upcoming_treatments', 'stats'],
                    });
                },
                onError: (err: any) => {
                    addToast({
    ...
    const handleReject = () => {
        ...
        setActionLoading(true);
        router.post(
            treatmentsReject.url(selectedTreatment.id),
                    ...
                    <div className="mx-auto flex max-w-7xl items-center justify-between">
                        <div>
                            ...
                            <p className="mt-0.5 text-sm text-stone-500">
                                {treatments.total} traitement
                                {treatments.total !== 1 ? 's' : ''} enregistrés
                            </p>
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                        ...
                        <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-xs font-medium text-stone-500">
                                        Coût moyen par poule
                                    </p>
                                    <p className="text-2xl font-bold text-stone-900">
                                        {formatCurrency(
                                            stats.average_cost_per_bird,
                                        )}
                                    </p>
                                    <p className="text-xs text-stone-400">
                                        {stats.active_birds} poules actives
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                {upcoming_treatments.map((t) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex cursor-pointer items-center justify-between rounded-lg border p-3 transition hover:shadow-md ${
                                                isCritical
                    ...
                    <div className="overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
                        ...
                        <div className="grid grid-cols-7 gap-px bg-stone-200">
                            {[
                                ...
                                <div
                                    key={day}
                                    className="bg-stone-50 px-3 py-2 text-center text-xs font-semibold text-stone-600"
                                >
                                    {day}
                                </div>
                            ))}
                        ...
                        <div className="grid grid-cols-7 gap-px bg-stone-200">
                            {monthDays.map((day) => {
                                ...
                                return (
                                    <div
                                        key={dateKey}
                                        className={`min-h-[100px] bg-white p-2 ${!isCurrentMonth ? 'opacity-40' : ''}`}
                                    >
                                            ...
                                            {dayTreatments.map((t) => {
                                                ...
                                                return (
                                                    <div
                                                        ...
                                                        className={`flex cursor-pointer items-center gap-1 rounded p-1 text-xs transition-all ${
                                                            isTodayDay
                                                                ? 'animate-pulse border border-blue-300 bg-blue-50'
                                                                : ''
                                                        } ${
                                                            isPastTreatment
                                                    ...
                                                    >
                                                        <span
                                                            className={`rounded p-0.5 ${getTypeColor(t.treatment_type)}`}
                                                        >
                                                            {getIcon(
                                                                t.treatment_type,
                                                            )}
                ...
                <div className="space-y-4 px-7 py-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <InfoRow
                            label="Date"
                            value={format(
                                parseISO(treatment.treatment_date),
                                'dd/MM/yyyy',
                            )}
                        />
                        <InfoRow
                                ...
                                <div className="flex gap-3">
                                    {canReject && (
                                        <button
                                            ...
                                            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-red-500 px-4 py-2 text-sm text-white hover:bg-red-600 disabled:opacity-40"
                                        >
                                            {actionLoading ? (
                                                '...'
                                            ) : (
                                    ...
                                    {canApprove && (
                                        <button
                                            ...
                                            className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2 text-sm text-white hover:bg-emerald-700"
                                        >
                                            {actionLoading ? (
                                                '...'
                                            ) : (
...
function InfoRow({ label, value, children }: any) {
    return (
        <div className="flex items-baseline gap-2">
            <span className="min-w-[80px] text-xs font-medium text-stone-500">
                {label} :
            </span>
